    ...
    var $chart = $('<div>', {
      ...
      'click': function(event) {
        if (!$(event.target).closest('.node').length) {
          ...
          //$chart.find(".content:first").html("111");
        }
      }
    });
    if ($.type(data) === 'object') {
      ...
      $.ajax({
        ...
        'dataType': 'json',
        'beforeSend': function () {
          //$chart.append('<i class="fa fa-circle-o-notch fa-spin spinner"></i>');//图标
        }
      })
...
/////////////位置
  ...
  function buildJsonDS($li) {
    var subObj = {
      'stationsName': $li.contents().eq(0).text().trim(),//name
  ...
  function loopChart($chart) {
    ...
    //$tr.attr("data-id":$tr.find('.node')[0].id);//shishi
    var subObj = { 'id': $tr.find('.node')[0].id };
    $tr.siblings(':last').children().each(function() {
    	//$(this).attr("data-id":$(this).parent().parent().attr("data-id"))//shishi
      if (!subObj.children) { subObj.children = []; }
